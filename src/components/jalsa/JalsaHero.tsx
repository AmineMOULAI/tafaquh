...
export default function JalsaHero({ lng, totalProjects, totalRecordings }: JalsaHeroProps) {
  ...
  return (
    <div className="relative text-center max-w-4xl mx-auto space-y-6 pt-6 pb-12">
      ...
      <motion.h1
        ...
        className={`text-4xl md:text-6xl lg:text-7xl font-bold tracking-tight text-gold-radiant drop-shadow-2xl ${isAr ? 'font-calligraphy' : 'font-display uppercase'}`}
      >
        {title}
      </motion.h1>
      ...
      >
        <div className="p-4 rounded-2xl bg-black/40 border border-gold/30 backdrop-blur-md">
        ...
        <div className="p-4 rounded-2xl bg-black/40 border border-gold/30 backdrop-blur-md">
          ...
          <span className="text-xs text-gray-300 font-amiri">{isAr ? 'تسجيلات صوتية متاحة' : 'Audio Recordings'}</span>
        </div>
        <div className="p-4 rounded-2xl bg-black/40 border border-gold/30 backdrop-blur-md">
          <span className="block text-2xl md:text-3xl font-bold text-gold font-mono">80+</span>
